...
public class PatternLockView extends View {
        ...
        for (int i = 0; i < sDotCount; i++) {
            ...
            for (int j = 0; j < sDotCount; j++) {
                ...
                drawCircle(canvas, (int) centerX, (int) centerY + translationY,
                        size, drawLookupTable[i][j], dotState.mAlpha, dotState);
            }
        }
    ...
    private void resetPattern() {
        ...
        for (int i = 0; i < sDotCount; i++) {
            for (int j = 0; j < sDotCount; j++) {
                // Cancel running inner circle animators
                if (mDotStates[i][j].mInnerCircleAnimator != null) {
                    mDotStates[i][j].mInnerCircleAnimator.cancel();
                    mDotStates[i][j].mInnerCircleAnimator = null;
                }
                mDotStates[i][j].mInnerCircleSize = 0;
            }
        }
        invalidate();
    }
    ...
    private void startDotSelectedAnimation(Dot dot) {
        ...
        float maxInnerCircleSize = mDotNormalSize - mPathWidth * 4;
        startInnerCircleAnimation(0, maxInnerCircleSize, mDotAnimationDuration,
                mLinearOutSlowInInterpolator, dotState, null);
    ...
    private void startInnerCircleAnimation(float start, float end, long duration,
                                           Interpolator interpolator, final DotState state,
                                           final Runnable endRunnable) {
        // Cancel any running animator before starting a new one
        if (state.mInnerCircleAnimator != null) {
            state.mInnerCircleAnimator.cancel();
        }
        ValueAnimator valueAnimator = ValueAnimator.ofFloat(start, end);
        state.mInnerCircleAnimator = valueAnimator;
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {

            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                state.mInnerCircleSize = (Float) animation.getAnimatedValue();
                invalidate();
            }
    ...
    private void handleActionDown(MotionEvent event) {
        // Clear all inner circles when starting a new draw
        for (int i = 0; i < sDotCount; i++) {
            for (int j = 0; j < sDotCount; j++) {
                mDotStates[i][j].mInnerCircleSize = 0;
            }
    ...
    private void drawCircle(Canvas canvas, float centerX, float centerY,
                            float size, boolean partOfPattern, float alpha, DotState dotState) {
        // Draw the stroke circle (outer circle)
        mDotPaint.setColor(getCurrentColor(partOfPattern));
        mDotPaint.setAlpha((int) (alpha * 255));
        mDotPaint.setStyle(Paint.Style.STROKE);
        mDotPaint.setStrokeWidth(mPathWidth);
        canvas.drawCircle(centerX, centerY, size / 2, mDotPaint);

        // Draw the inner filled circle if it has size
        if (dotState.mInnerCircleSize > 0) {
            mDotPaint.setStyle(Paint.Style.FILL);
            mDotPaint.setStrokeWidth(0);
            canvas.drawCircle(centerX, centerY, dotState.mInnerCircleSize / 2, mDotPaint);
        }
    }
    ...
    public static class DotState {
        ...
        float mAlpha = 1.0f;
        float mSize;
        float mInnerCircleSize = 0.0f; // New property for inner circle animation
        float mLineEndX = Float.MIN_VALUE;
        float mLineEndY = Float.MIN_VALUE;
        ValueAnimator mLineAnimator;
        ValueAnimator mInnerCircleAnimator; // Animator for inner circle
    }
}
